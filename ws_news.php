...
$master  = WebSocket("192.168.0.6",7000);
...
function process($user,$msg){
  ...
  switch($action){
    ...
    case "news"  : sendNews($user->socket);				    break;
    default      : send($user->socket,$action." not understood");           break;
  }
}

function sendNews($client) {
  send($client, "ok, will send news");
  sleep(5);
  send($client, "Headline: flying monkeys invade!");
  sleep(5);
  send($client, "Headline: flying leopards eat flying monkeys!");
  sleep(5);
  send($client, "Headline: Outer-space goo dissolves monkeys and leopards!");
}
